import type { Work, WorkPlatform, WorkType } from "./types";
...
function rowsToObjects(text: string): Row[] {
...
async function fetchSheet(name: string, gid: string): Promise<Row[]> {
...
/** Google Sheet → Work[] (런타임/빌드 공통) */
export async function buildWorksCatalogFromSheets(): Promise<Work[]> {
  const [
    workRows,
    scheduleRows,
    serialDayRows,
  ...
  for (let i = 0; i < workRows.length; i += 1) {
    ...
    const wtype: WorkType = row.content_type === "웹툰" ? "webtoon" : "anime";
    ...
    const otherStaff: string[] = [];
    for (const link of staffLinks) {
      const name = staffById.get(link.staff_id);
      if (!name) continue;
      const role = link.role || "";
      if (role.includes("감독")) directors.push(name);
      else if (role.includes("글") || role.includes("원작") || role.includes("각본")) {
        writers.push(name);
      } else if (role.includes("그림") || role.includes("작화")) {
        illustrators.push(name);
      } else {
        otherStaff.push(`${name}${role ? `(${role})` : ""}`);
      }
    }
